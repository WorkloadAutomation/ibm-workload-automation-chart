...
sed 's/{{ .cv.release }}/'$chartRelease'/g' $preinstallDir/db.yaml | kubectl create -f -
sed 's/{{ .cv.release }}/'$chartRelease'/g' $preinstallDir/db-service.yaml | kubectl create -f -	
sed 's/{{ .cv.release }}/'$chartRelease'/g' $preinstallDir/secret.yaml | kubectl create -f -
sed 's/{{ .cv.release }}/'$chartRelease'/g' $preinstallDir/role.yaml | kubectl create -f -	
kubectl create rolebinding wa-pod-rolebinding --role=wa-pod-label --group=system:serviceaccounts:${CV_TEST_NAMESPACE}
kubectl describe pod ${chartRelease}-wa-db2
kubectl describe service test-wa-db2-service
kubectl get secret wa-pwd-secret
kubectl get rolebinding wa-pod-rolebinding