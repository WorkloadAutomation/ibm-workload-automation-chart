...
kubectl exec -ti ${chartRelease}-waserver-0 pidof java
...
manageError $?
echo "Verify main console processes are running"
...
manageError $SUCC
echo "Verify if the external agent is up and linked"
kubectl exec -ti ${chartRelease}-waserver-0 -- bash -c ". /opt/wa/TWS/tws_env.sh && conman sc" | grep "WA_AGT" | grep "LBI J" > /dev/null 2>&1
manageError $?
echo "Submit a job on the external agent"
kubectl exec -ti ${chartRelease}-waserver-0 -- bash -c ". /opt/wa/TWS/tws_env.sh && conman sbd WA_AGT#'ls';streamologon=wauser"
manageError $?
...
while [ $(date +%s) -lt $timeout -a $SUCC -ne 0 ]; do
    kubectl exec -ti ${chartRelease}-waserver-0 -- bash -c ". /opt/wa/TWS/tws_env.sh && conman sj WA_AGT#/JOBS.LS" | grep SUCC > /dev/null 2>&1
    SUCC=$?
    sleep 5
done
manageError $SUCC
